...
export function scoreItem(item: Omit<RankItem, 'score'>, feed: RankFeed, opts?: { boosts?: [RegExp, number][]; matchReport?: RegExp[]; now?: number }): number;
export function dedupe<T extends RankItem>(items: T[]): T[];
export function selectPerTopic<T extends RankItem>(items: T[], opts: { perTopic: number; maxAgeHours: Record<string, number>; seen?: Record<string, string>; now?: number; topics: string[]; maxPerPublisher?: number; maxForeignShare?: number }): Record<string, (T & { isBackground: boolean })[]>;
